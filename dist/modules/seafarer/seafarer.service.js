...
let SeafarerService = class SeafarerService {
    supabaseService;
    invoicesService;
    usersFile = path.join(process.cwd(), 'users_data.json');
    seafarersFile = path.join(process.cwd(), 'seafarers_local_data.json');
    coursesFile = path.join(process.cwd(), 'courses_data.json');
    purchasesFile = path.join(process.cwd(), 'partner_purchases_data.json');
    enrollmentsFile = path.join(process.cwd(), 'local_enrollments_data.json');
    ...
    async getDashboard(userId) {
        ...
        const purchases = this.readJsonFile(this.purchasesFile, []).filter((p) => p.seafarer_id === userId);
        ...
        const activeEnrollment = enrollments.find((e) => e.status === 'Processing') || (enrollments.length > 0 ? enrollments[0] : null);
        const completedCount = enrollments.filter((e) => e.status === 'Completed').length;
        const localUsers = this.readJsonFile(this.usersFile, []);
        const localSeafarers = this.readJsonFile(this.seafarersFile, []);
        ...
        return {
            ...
            courses: {
                active: activeEnrollment
                    ? {
                        name: activeEnrollment.Course?.name || 'Basic Safety Training',
                        code: activeEnrollment.Course?.code || 'BST',
    ...
    async getAllCourses() {
        ...
        const localCourses = this.readJsonFile(this.coursesFile, []);
    ...
    async getMyEnrollments(userId) {
        ...
        if (dbEnrollments.length > 0) {
            const mappedDb = dbEnrollments.map((e) => ({
                id: e.id,
        ...
        const localEnrollments = this.readJsonFile(this.enrollmentsFile, []);
        const userEnrollments = localEnrollments.filter((e) => e.userId === userId || !e.userId);
        if (userEnrollments.length > 0) {
            const allCourses = await this.getAllCourses();
            return userEnrollments.map((e) => {
                const foundCourse = allCourses.find((c) => c.id === e.courseId || c.code === e.courseId) || e.course || {
                    id: e.courseId || 'c-001',
    ...
    async enrollInCourse(userId, courseId, referralCode) {
        ...
        if (!course) {
            throw new common_1.BadRequestException('Course not found in training registry');
        }
        const localEnrollments = this.readJsonFile(this.enrollmentsFile, []);
        const existing = localEnrollments.find((e) => (e.userId === userId || !e.userId) &&
            (e.courseId === course.id || e.courseId === course.code));
        if (existing) {
            throw new common_1.BadRequestException('Already enrolled in this course');
        }
        const enrollmentId = (0, crypto_1.randomUUID)();
        const newEnrollment = {
            id: enrollmentId,
            userId,
            courseId: course.id,
    ...
    async updateCourseProgress(userId, courseId, progress) {
        ...
        try {
            const purchases = this.readJsonFile(this.purchasesFile, []);
            let updatedPurchase = false;
            for (const p of purchases) {
                if ((p.seafarer_id === userId || !p.seafarer_id) &&
        ...
        try {
            const localEnrollments = this.readJsonFile(this.enrollmentsFile, []);
            const existingIdx = localEnrollments.findIndex((e) => (e.userId === userId || !e.userId) &&
    ...
    async updateUserProfile(userId, details) {
        ...
        if (sf) {
            ...
            if (details.nationality)
                sf.nationality = details.nationality;
            if (details.indosNumber)
                sf.indosNumber = details.indosNumber;
            if (details.address)
