...
let MasterService = class MasterService {
    ...
    settlementsFile = path.join(process.cwd(), 'settlements_data.json');
    ...
    async getReportsData(days) {
        const courses = this.readJsonFile(this.coursesFile, this.getStandardCoursesList());
        const purchases = this.readJsonFile(this.purchasesFile, []);
        const reports = courses.map((c, index) => {
            const coursePurchases = purchases.filter((p) => p.course_code === c.code || p.course_name === c.name || p.course_id === c.id);
    ...
    async createCourse(dto) {
        const payload = {
            id: `c-${(0, crypto_1.randomUUID)().substring(0, 8)}`,
    ...
    async updateCourse(id, dto) {
        const currentCourses = await this.getCourses();
        const idx = currentCourses.findIndex((c) => c.id === id || c.code === id);
        if (idx === -1) {
    ...
    async deleteCourse(id) {
        ...
        currentCourses = currentCourses.filter((c) => c.id !== id && c.code !== id);
        this.writeJsonFile(this.coursesFile, currentCourses);
        try {
